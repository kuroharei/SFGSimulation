...
def Expection(func, Disfunc=[0], W=[0], Lower=[0], Upper=[0], N=[100], A=[1], X=[0]):
    return sum(map((lambda disfunc, w, lower, upper, n, a, x : a * func(x) if disfunc == 0 else \
                                                               a * sum([func((upper - lower) / n * i) * disfunc((upper - lower) / n * i, w, lower, upper, x) * (upper - lower) / n for i in range(n)]) \
...
class Sample:
    def __init__(self, lamda, beta, n1, n2, theta_distribution=['delta'], psi_distribution=['uniform']) -> None:
        ...
        self.omega = list(map((lambda lamda : 2 * np.pi * c / lamda * 1e7), self.lamda))
        self.beta = [np.arcsin(self.lamda[0] * (np.sin(beta[0]) / self.lamda[1] + np.sin(beta[1]) / self.lamda[2]))] + beta
        self.n1 = n1
        self.n2 = n2
        self.n_ = list(map(lambda n1, n2 : np.sqrt((n1 ** 2 + n2 ** 2 + 4) / (2 * (n1 ** -2 + n2 ** -2 + 1))), self.n1, self.n2))
        self.gamma = list(map(lambda beta, n1, n2 : np.arcsin(np.sin(beta) * n1 / n2), self.beta, self.n1, self.n2))
        self.theta_distribution = np.array(list(map((lambda x : distribution[x]), theta_distribution)))
        self.psi_distribution = np.array(list(map((lambda x : distribution[x]), psi_distribution)))
    ...
    def sspssParam(self, squarecospsi, squaresinpsi):
        self.sspssd = self.Lyy(0) * self.Lyy(1) * self.Lzz(2) * np.sin(self.beta[2]) * self.yyzssParam(squarecospsi, squaresinpsi)[0]
    ...
    def spsssParam(self, squarecospsi, squaresinpsi):
        self.spsssd = self.Lyy(0) * self.Lzz(1) * self.Lyy(2) * np.sin(self.beta[1]) * self.yzyssParam(squarecospsi, squaresinpsi)[0]
    ...
    def pssssParam(self, squarecospsi, squaresinpsi):
        self.pssssd = self.Lzz(0) * self.Lyy(1) * self.Lyy(2) * np.sin(self.beta[0]) * self.zyyssParam(squarecospsi, squaresinpsi)[0]
    ...
    def sspasParam(self, squarecospsi, squaresinpsi):
        self.sspasd = self.Lyy(0) * self.Lyy(1) * self.Lzz(2) * np.sin(self.beta[2]) * self.yyzasParam(squarecospsi, squaresinpsi)[0]
    ...
    def spsasParam(self, squarecospsi, squaresinpsi):
        self.spsasd = self.Lyy(0) * self.Lzz(1) * self.Lyy(2) * np.sin(self.beta[1]) * self.yzyasParam(squarecospsi, squaresinpsi)[0]
    ...
    def pssasParam(self, squarecospsi, squaresinpsi):
        self.pssasd = self.Lzz(0) * self.Lyy(1) * self.Lyy(2) * np.sin(self.beta[0]) * self.zyyasParam(squarecospsi, squaresinpsi)[0]
...
class Cinfv(Sample):
    def __init__(self, lamda, beta, n1, n2, r, theta_distribution=['delta'], psi_distribution=['uniform']) -> None:
...
class C2v(Sample):
    def __init__(self, lamda, beta, n1, n2, r, tau, theta_distribution=['delta'], psi_distribution=['uniform']) -> None:
        super(C2v, self).__init__(lamda, beta, n1, n2, theta_distribution, psi_distribution)
        self.r = r
        self.tau = tau
        self.Ra = (1 + self.r - (1 - self.r) * np.cos(self.tau)) / (1 + self.r + (1 - self.r) * np.cos(self.tau))
...
class C3v(Sample):
    def __init__(self, lamda, beta, n1, n2, R=0, theta_distribution=['delta'], psi_distribution=['uniform']) -> None:
